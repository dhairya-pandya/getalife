  ...
  return (
    <div className="w-full max-w-md">
      ...
      <p className="text-center text-xs text-gray-500 mt-6">
        Already a redditor? <a href="/" className="text-blue-400 hover:underline">Log In</a>
      </p>
    </div>
  );
};
...
const OtpVerification = ({ onSubmit, email, error }) => {
  ...
  return (
    <div className="w-full max-w-md text-center">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          className="w-full mt-4 bg-blue-600 text-white font-semibold py-2 rounded-full hover:bg-blue-700 transition-colors duration-200"
        >
          Verify
        </button>
      </form>
...
const InterestSelection = ({ onSubmit }) => {
  ...
  return (
    <div className="w-full max-w-2xl text-center">
      ...
      <button
        ...
        className="w-full max-w-xs bg-blue-600 text-white font-semibold py-2 rounded-full hover:bg-blue-700 transition-colors duration-200 disabled:bg-gray-500 disabled:cursor-not-allowed"
...
function SignUpPage() {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // State to manage OTP sending process
  ...
  const handleSignUpSubmit = (data) => {
    ...
    // Simulate sending OTP with a network delay
    console.log("Simulating sending OTP to", data.email);
    setTimeout(() => {
      setStep(2);
      // No need to reset isSubmitting, as this component will unmount
    }, 2000); // 2-second delay
  };
  ...
  const handleInterestSubmit = (interests) => {
    const finalUserData = { ...userData, interests };
    console.log("Final User Data with Interests:", finalUserData);
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return <SignUpForm onSubmit={handleSignUpSubmit} isSubmitting={isSubmitting} />;
      case 2:
        return <OtpVerification onSubmit={handleOtpSubmit} email={userData?.email} error={otpError} />;
      case 3:
        return <InterestSelection onSubmit={handleInterestSubmit} />;
      default:
        return <SignUpForm onSubmit={handleSignUpSubmit} isSubmitting={isSubmitting} />;
    }
  };
